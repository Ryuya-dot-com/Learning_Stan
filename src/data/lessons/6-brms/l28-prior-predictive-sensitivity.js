export default {
  ...
  pages: [
    ...
    },
    {
      ...
      code: "wide_priors <- c(\n  set_prior(\"normal(70, 30)\", class = \"Intercept\"),\n  set_prior(\"normal(0, 10)\", class = \"b\"),\n  set_prior(\"exponential(1.0 / 30)\", class = \"sigma\")\n)\n\nfit_a <- update(prior_fit, sample_prior = \"no\", prior = priors)\nfit_b <- update(prior_fit, sample_prior = \"no\", prior = wide_priors)\nposterior_summary(fit_a, pars = \"b_sleep_c\")\nposterior_summary(fit_b, pars = \"b_sleep_c\")",
      verify: { mode: "manual", reason: "brmsとCmdStan環境で実行する例のため" },
      a: ["「defaultを使ったから影響はない」とは言えません。`get_prior()`でparameter classを確認し、なぜその範囲を比較したか、予測と結論がどう変わったかを記録します。"],
    },
  ],
  ex: [
    { id: "l28-q01", revision: 1, k: "choice", q: "`sample_prior = \"only\"`の主な目的はどれですか。", opts: ["priorから仮想の応答を生成して含意を確かめる", "観測値を二度使って係数を安定させる", "divergenceを必ず消す", "因果効果を識別する"], ans: 0, why: "事前予測では尤度による更新前に、priorと尤度の組合せがどんな値を作るかを調べます。", hint: "観測データで更新する前に何を見たいか考えます。" },
    { id: "l28-q02", revision: 1, k: "choice", q: "傾きのpriorを`normal(0, 5)`から`normal(0, 1)`へ替え、ほかを同じにしました。事前予測の回帰線はどう変わりやすいですか。", opts: ["傾きが0付近へより集中し、極端な線が減る", "得点が必ず70点になる", "残差が必ず0になる", "観測値が増える"], ans: 0, why: "傾きのpriorだけを狭めると、許す傾きの範囲が0付近へ集中します。interceptや残差を直接固定するものではありません。", hint: "変更したのは傾き`b`のpriorだけです。" },
    { id: "l28-q03", revision: 1, k: "reflect", q: "prior predictiveで−20点と120点の複製得点が多数出ました。次に調べることを二つ書いてください。", minLength: 70, rubric: ["intercept・傾き・sigmaのpriorの尺度を点検している", "正規尤度が得点の支持範囲を越えることを指摘している", "別の応答尺度または生成過程を候補として挙げている"], example: "まずinterceptとsigmaのpriorが得点尺度に対して広すぎないかを分けて確認する。正規尤度は0〜100の範囲を自動では守らないので、境界への張り付き方を調べ、正答数なら二項、順序評定なら順序モデルなどを候補にする。" },
    { id: "l28-q04", revision: 1, k: "reflect", q: "次の分析計画をレビューしてください。何が不足していますか。", code: "fit <- brm(score ~ sleep_c, data = dat)\n# default priorなので、priorの影響は報告しない", lang: "R", verify: { mode: "manual", reason: "レビュー用の不十分な分析計画を含むため" }, minLength: 70, rubric: ["default priorでも尺度とparameter classの確認が必要と指摘している", "事前予測を提案している", "合理的な代替priorによる感度分析を提案している"], example: "default priorもデータ尺度とモデルに依存するので、影響がない根拠にはならない。`get_prior()`で対象を確認し、proper priorで事前予測を行い、少なくとも一つの合理的な代替priorで係数と予測を比べる。" },
    { id: "l28-q05", revision: 1, k: "reflect", q: "代替priorでも`P(b_sleep_c > 0 | y) = 0.97`でした。「睡眠を1時間増やす介入は得点を改善する」と書けるか説明してください。", minLength: 70, rubric: ["事後確率が条件付き関連を表すと述べている", "交絡または逆因果の可能性に触れている", "介入効果には別の設計・仮定が必要と区別している"], example: "この値は指定した観察モデルで睡眠と得点が正に関連する確率である。睡眠を増やす介入の効果には、交絡や逆因果を扱う設計と仮定が別に必要なので、そのまま因果効果とは書けない。" },
