export default {
  ...
  ex: [
    { k: "choice", q: "divergenceが確認されたfitについて、最初に避けるべきことはどれですか。", opts: ["推定値をそのまま結論に使う", "どのparameter付近で起きるか調べる", "モデルの尺度とpriorを確認する", "再現可能なseedを記録する"], ans: 0, why: "divergenceは事後分布の探索が不十分な可能性を示します。推定値を解釈する前に、モデル化と計算を調べます。", hint: "計算が十分に探索できたか不明なとき、結論を先に出せるでしょうか。" },
    { k: "choice", q: "条件別中央値のPPCで、一方の条件の複製中央値だけが観測中央値より一貫して低いと分かりました。最も直接いえることはどれですか。", opts: ["その条件の中央値をモデルが十分に再現していない疑いがある", "条件効果は因果効果である", "全体の平均は必ず正しい", "chain数を増やせば中央値は自動的に合う"], ans: 0, why: "このPPCは選んだ統計量である条件別中央値の再現を検査しています。因果性やほかの特徴を直接保証しません。", hint: "PPCは、あらかじめ選んだどの特徴を比べているかに答えます。" },
    { k: "reflect", q: "LOOで候補Bが少し良かったが、差の標準誤差も大きく、条件別PPCではBが遅い反応を再現できませんでした。比較結果を二文で報告してください。", minLength: 80, rubric: ["LOOの差に不確実性があることを述べている", "PPCで再現できない特徴を具体的に述べている", "順位だけで真のモデルや因果効果と結論していない"], example: "同じ観測単位への予測では候補Bがわずかに良かったが、LOO差には大きな不確実性があった。さらにBは条件別の遅い反応を再現できなかったため、Bを唯一の正しいモデルとは扱わず、予測目的に照らして改良または併記を検討する。" },
    { k: "reflect", q: "次の比較コードをレビューしてください。なぜ`loo_compare()`を実行する前に止めますか。", code: "fit_a <- brm(rt_ms ~ condition, data = people_2025)\nfit_b <- brm(rt_ms ~ condition + baseline_z, data = people_2026)\nloo_compare(loo(fit_a), loo(fit_b))", lang: "R", verify: { mode: "manual", reason: "レビュー用の比較不能なコードを含むため" }, minLength: 70, rubric: ["二つのfitが異なるデータを使っていると指摘している", "同じ観測単位と予測課題をそろえる必要を述べている", "年度差はモデル差と区別できないと説明している"], example: "二つのモデルが別年度の別データでfitされているので、LOO差は式の違いだけを表さない。同じ対象・同じ観測単位・同じ予測課題でfitし直してから比較する。" },
    { k: "reflect", q: "LOOで候補Aより候補Bの予測性能が良好でした。「基礎得点を加えることが反応時間を変えた」と書けるか説明してください。", minLength: 70, rubric: ["LOOが予測性能の比較であると述べている", "予測子を加える行為と因果介入を区別している", "因果主張には設計または追加仮定が必要と述べている"], example: "書けない。LOOは基礎得点を含む候補が未見データをよりよく予測したことを示すにとどまる。基礎得点を操作する因果効果には、交絡や時間順序を扱う設計・仮定が別に必要である。" },
  ],
