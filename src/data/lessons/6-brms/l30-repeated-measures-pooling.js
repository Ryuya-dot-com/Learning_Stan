export default {
  ...
  pages: [
    ...
    },
    {
      ...
      code: "# 既存参加者: その人の参加者効果を使える\nposterior_predict(fit, newdata = existing_trials)\n\n# 新参加者: 参加者分布から新しいずれを生成する設定を選ぶ\n# 予測前に「既存の人か、新しい人か」を文章で決める",
      verify: { mode: "manual", reason: "brmsとCmdStan環境で実行する例のため" },
      a: ["random interceptを入れても、順序効果、刺激差、欠測、因果性が自動で解決するわけではありません。必要なら、それぞれの観測過程を式へ加えるか、設計で扱います。"],
    },
  ],
  ex: [
    { k: "choice", q: "`rt_ms ~ condition + (1 + condition | participant)`で、参加者jの条件効果を表すのに最も近いものはどれですか。", opts: ["全体の条件効果と参加者jのずれの和", "残差`sigma`だけ", "参加者jの切片だけ", "R-hat"], ans: 0, why: "varying slopeを含むので、参加者jの条件効果は全体の傾きにその人の傾きのずれを加えたものです。", hint: "`1 + condition`のうち、conditionがどの効果に対応するか考えます。" },
    { k: "choice", q: "1試行しかない参加者の条件効果を、no poolingと階層モデルで推定しました。階層モデルの推定は一般にどこへ近づきやすいですか。", opts: ["全参加者の条件効果の分布", "必ず0", "その参加者の唯一の観測値だけ", "最も試行数の多い参加者の値そのもの"], ans: 0, why: "情報が少ない参加者ほど全体分布から強く情報を借ります。ただし全員が同じ推定値になるわけではありません。", hint: "部分プーリングでは、少ない情報をどこから補うか考えます。" },
    { k: "reflect", q: "新参加者の次の試行を予測するとき、既存参加者の参加者別drawをそのまま使えない理由を説明してください。", minLength: 70, rubric: ["新参加者には観測済みの個人効果がないと述べている", "群レベルの分布から新しい効果を生成する必要に触れている", "既存参加者の予測とは別の予測課題だと区別している"], example: "新参加者には、その人の過去試行から推定した切片や条件効果がない。したがって既存参加者の個人効果を流用せず、参加者全体の分布から新しいずれを生成する。これは既存参加者の次試行を予測する問いとは異なる。" },
    { k: "reflect", q: "次のformulaをレビューしてください。参加者内の条件差を参加者ごとに推定したい場合、何が足りませんか。", code: "brm(rt_ms ~ condition + (1 | participant),\n    data = trials, family = gaussian())", lang: "R", minLength: 70, rubric: ["participantごとの切片だけが入っていると指摘している", "参加者ごとの条件効果にはvarying slopeが必要と述べている", "`(1 + condition | participant)`または同等の式を提案している"], example: "この式は参加者ごとの基準反応時間は許すが、条件効果は全員で同じと仮定する。参加者別の条件差を扱うなら`(1 + condition | participant)`のようにconditionのvarying slopeを加える。" },
    { k: "reflect", q: "反復測定の階層モデルで条件差の事後分布が得られました。「条件が反応時間を因果的に変えた」と書けるか説明してください。", minLength: 70, rubric: ["階層モデルが依存構造を扱うことを述べている", "因果性は割付けや交絡などの設計・仮定に依存すると述べている", "モデル構造だけで因果識別にならないと区別している"], example: "階層モデルは同じ参加者の試行が似る構造を表すが、それだけで因果効果を識別しない。条件の無作為化、順序効果、交絡の扱いなど設計と仮定が別に必要である。" },
