export default {
  ...
  ex: [
    { id: "l31-q01", revision: 1, k: "choice", q: "0/1の正答`correct`をBernoulliモデルで扱うとき、最も適切なデータ制約はどれですか。", opts: ["整数で0または1", "任意の実数", "整数で1〜5", "必ず平均0・分散1"], ans: 0, why: "Bernoulli観測の支持範囲は0と1です。fitの前に値域を確認します。", hint: "正答・誤答を何通りの値で記録しているか考えます。" },
    { id: "l31-q02", revision: 1, k: "choice", q: "offset以外が同じで、観察時間が30分から60分へ二倍になりました。Poissonモデルの期待件数はどうなりますか。", opts: ["二倍になる", "半分になる", "変わらない", "必ず0になる"], ans: 0, why: "log offsetは既知の曝露量を表します。率が同じなら、観察時間が二倍で期待件数も二倍です。", hint: "件数は率と観察時間の積として考えます。" },
    { id: "l31-q03", revision: 1, k: "reflect", q: "1〜5の疲労評定について、平均評定だけでなくカテゴリ確率も報告する理由を書いてください。", minLength: 70, rubric: ["順序は使えるがカテゴリ間隔が等しいとは限らないと述べている", "条件別の各カテゴリ確率が応答分布を示すと述べている", "平均だけでは異なる分布を隠し得ることに触れている"], example: "1〜5は順序を持つが、隣り合う数値の間隔が同じとは限らない。条件別のカテゴリ確率を示せば、平均が似ていても高い評定へ移るのか、両端が増えるのかを区別できる。" },
    { id: "l31-q04", revision: 1, k: "reflect", q: "次の件数モデルをレビューしてください。実行前に止めるべき行と理由を説明してください。", code: "counts$observation_minutes <- c(10, 0, 30, -5)\nbrm(lapses ~ condition + offset(log(observation_minutes)),\n    data = counts, family = poisson())", lang: "R", minLength: 70, rubric: ["0または負の観察時間では`log()`が使えないと指摘している", "曝露量が正であることを検査する提案をしている", "削除だけでなく記録・原因確認に触れている"], example: "0と−5は`log(observation_minutes)`を計算できないのでfit前に止める。観察時間が正かを検査し、入力ミス、未観察、記録単位のどれかを問題として記録してから処理を決める。" },
    { id: "l31-q05", revision: 1, k: "reflect", q: "条件Bの予測正答確率が条件Aより高いことを示すモデルが得られました。「Bは学習効果がある」と書けるか説明してください。", minLength: 70, rubric: ["予測確率がモデル上の関連または差であると述べている", "因果主張には割付け・交絡などの根拠が必要と述べている", "尤度の選択と因果識別を区別している"], example: "予測正答確率の差だけでは、Bが学習を生んだとは言えない。応答の尤度を適切にしたことと、条件割付けや交絡を扱って因果効果を識別することは別である。" },
