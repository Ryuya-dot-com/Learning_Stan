export default {
  ...
  ex: [
    { k: "choice", q: "生成Stanコードの`generated quantities`を読むとき、最も適切な見方はどれですか。", opts: ["drawごとに追加の量を計算する場所として読む", "観測データを受け取る場所として読む", "priorだけを書く場所として読む", "R-hatの値を手で入力する場所として読む"], ans: 0, why: "`generated quantities`では、parameterを新たに推定せず、drawごとに追加の量を計算できます。予測や観測別log likelihoodがここにある設定もありますが、`brms`では設定によりR側で必要なときに計算することもあります。", hint: "data・parameters・model・generated quantitiesの役割を分けます。" },
    { k: "choice", q: "既存参加者の次試行を当てる課題から、新参加者の試行を当てる課題へ変えたとき、最も直接変わるものはどれですか。", opts: ["使える参加者別情報と、交差検証で外す単位", "応答の単位だけ", "観測済みデータの行数だけ", "因果効果の定義だけ"], ans: 0, why: "新参加者には観測済みの参加者別効果がないため、予測時の情報と評価単位を変える必要があります。", hint: "誰のどのデータをまだ見ていない状態にするか考えます。" },
    { k: "reflect", q: "parameter recoveryとSBCの違いを、生成するparameterの出所と繰り返し回数に触れて説明してください。", minLength: 90, rubric: ["recoveryでは既知の値を自分で固定して仮想データを作ると述べている", "SBCではpriorからparameterとデータを繰り返し生成すると述べている", "一回の回復成功だけでは手続きの校正を保証しないと述べている"], example: "parameter recoveryでは、例えば`true_beta = 0.4`を固定して仮想データを作り、fitがその値を回復するかを見る。SBCではparameterもpriorから引き、同じ流れを多数回繰り返して真値の事後順位を調べる。一回の成功だけでは系統的な偏りを見つけられない。" },
    { k: "reflect", q: "次のStanコードをレビューしてください。`log_lik[n]`として観測ごとの寄与を保存したいとき、何を直しますか。", code: "for (n in 1:N) {\n  log_lik[n] = normal_lpdf(y | mu, sigma);\n}", lang: "Stan", minLength: 70, rubric: ["ループ内なのに全ベクトル`y`と`mu`を毎回使っていると指摘している", "`y[n]`と`mu[n]`を使う修正を示している", "観測ごとの寄与を保存する目的に触れている"], example: "このままでは同じ全データの対数尤度をN回複製してしまう。`log_lik[n] = normal_lpdf(y[n] | mu[n], sigma);`とし、各列が観測nの寄与になるようにする。" },
    { k: "reflect", q: "SBCの順位が一様に近く、recoveryも良好でした。「現実の研究モデルは正しい」と結論してよいか説明してください。", minLength: 80, rubric: ["SBCとrecoveryが想定した生成過程での手続き検査であると述べている", "現実の観測過程・prior・モデル仕様が正しいこととは別と述べている", "PPCや感度分析、研究設計など追加の点検に触れている"], example: "結論してよいとは限らない。SBCとrecoveryは、自分が想定した生成過程のもとで実装と推定手続きが整合的かを試す検査である。現実の尤度、prior、欠測、因果設計が妥当かはPPC、感度分析、研究設計でも別に点検する。" },
