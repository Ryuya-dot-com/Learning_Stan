...
legend("topright", c("90%区間", "実用上の基準: 2点"),
  ...
  ex: [
    { k: "choice", q: "標本平均が2.1点、priorがNormal(0, 4)で、データが多くばらつきも小さいときの事後平均として最も妥当なのはどれですか?", opts: ["必ず0点", "必ず2.1点", "通常は0点と2.1点の間で、2.1点に近い", "必ず4点"], ans: 2, why: "尤度が鋭いほどデータの重みが大きくなります。priorを無視して必ず標本平均になるわけでも、prior平均や標準偏差そのものになるわけでもありません。", hint: "事後分布はpriorと尤度の両方を反映します。" },
    { k: "choice", q: "`dnorm(mu_grid, 0, 4)`を`dnorm(mu_grid, 3, 1)`へ変え、同じデータで更新すると、事後分布はどう変わりやすいですか?", opts: ["必ず3点になる", "3点側へ動き、より狭いpriorの影響を受ける", "標本平均だけで決まるので変わらない"], ans: 1, why: "priorの中心と広がりが変われば事後にも影響します。ただし尤度が非常に鋭ければ、変化は小さいことがあります。", hint: "データ後もpriorを完全に捨てるわけではありません。" },
    { k: "reflect", q: "休憩あり−なしの母平均差について、モデルに条件づく不確実性を含む結果文を1〜2文で書いてください。", minLength: 55, rubric: ["muが休憩あり−なしの母平均差であることを示している", "モデルとデータに条件づく表現を含めている", "信用区間または実用上の閾値を超える確率を含めている"], example: "参加者内差を正規分布で表したこのモデルでは、休憩あり−なしの母平均差muの事後中央値と90%信用区間を報告する。これは指定したpriorとこの24人のデータに条件づく要約である。" },
    { k: "reflect", q: "得点尺度が0〜20点なのに`mu ~ Normal(0, 100)`を「無情報prior」と呼んだ研究者へ、短いレビューコメントを書いてください。", minLength: 55, rubric: ["priorの尺度を得点尺度と結び付けている", "広いpriorが中立性を保証しないと説明している", "事前予測または根拠の提示を求めている"], example: "0〜20点の尺度で100点規模の差を許す根拠と、そのpriorが作る複製得点を示してください。広いpriorであることは、現実的または中立である証拠ではありません。" },
    { k: "choice", q: "休憩を自分で選んだ人ほど高得点だったとき、事後分布から「休憩を強制すれば全員の得点が上がる」と言えますか?", opts: ["言える。事後分布が不確実性をすべて扱うから", "言えない。自己選択などの交絡はベイズ更新だけでは消えない", "言える。90%信用区間が正なら因果である"], ans: 1, why: "ベイズ更新は指定した観測モデル内の不確実性を表します。自己選択、疲労、課題難易度などの交絡を消すには、設計や追加の因果仮定が必要です。", hint: "関連を推定することと、介入の因果効果を識別することを分けます。" },
  ],
  practice: {
    title: "成果物チェック",
