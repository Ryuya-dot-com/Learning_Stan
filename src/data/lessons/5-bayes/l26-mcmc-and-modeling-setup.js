...
run_record`,
  ...
  ex: [
    { k: "choice", q: "warmupの主な役割として最も適切なのはどれですか?", opts: ["最終報告に使うdrawを増やす", "HMCが探索に使う設定を調整する", "観測データを追加する", "priorを自動で選ぶ"], ans: 1, why: "warmupはstep sizeなどを適応させる期間です。通常はwarmup後のsampling drawを事後要約に用います。", hint: "warmupはsamplingの前に探索の準備をする期間です。" },
    { k: "choice", q: "4 chain、iter = 2000、warmup = 1000を指定したとき、warmup後のdrawは合計でいくつですか?", opts: ["1000", "2000", "4000", "8000"], ans: 2, why: "各chainは2000回のうち1000回をwarmupに使うため、sampling drawは1000回です。4 chainで4000 drawになります。", hint: "1 chainに残るdraw数を出してからchain数を掛けます。" },
    { k: "reflect", q: "R-hatが1.00でも、推定値を直ちに研究結論へ使えない理由を二つ書いてください。", minLength: 55, rubric: ["R-hatがchain間の整合性に関する指標だと述べている", "divergence、ESS、またはMonte Carlo精度を別に確認すると述べている", "観測過程、prior、PPCなどモデル妥当性の確認を挙げている"], example: "R-hatはchainが同じ分布を探索しているかの一指標であり、divergenceやESSを代替しない。また、良いR-hatは指定した尤度やpriorが研究上妥当である証拠ではないので、PPCと観測過程も確認する。" },
    { k: "reflect", q: "install_cmdstanが失敗したとき、再試行前に残すべき情報と、避けるべき行動を短く書いてください。", minLength: 55, rubric: ["エラーメッセージまたは失敗した工程を残している", "OS、R版、パッケージ版、toolchain診断の情報を挙げている", "原因を確認せず設定を無作為に変えないと述べている"], example: "エラーメッセージ、OS、R版、cmdstanr版、check_cmdstan_toolchainの結果を残す。どの工程で失敗したかを分け、原因を確認せずコンパイラ設定やsampling設定を次々に変えない。" },
    { k: "choice", q: "samplingが最後まで完走し、警告もなかったときに直接言えることはどれですか?", opts: ["因果効果が確認された", "モデルが真である", "指定した計算が完走した。診断とモデル検査は別に続ける", "priorの影響がない"], ans: 2, why: "完走は必要な段階ですが、計算診断、PPC、予測比較、研究設計の妥当性を保証しません。", hint: "実行できたことと研究上正しいことを分けます。" },
  ],
  practice: {
    title: "成果物チェック",
