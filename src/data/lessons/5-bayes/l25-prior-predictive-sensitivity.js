...
posterior_summary(fit_b, pars = "b_sleep_c")`,
  ...
  ex: [
    { id: "imitate-q01", revision: 1, k: "choice", q: "得点が0〜100点、睡眠時間が通常4〜12時間のとき、betaへ`normal(0, 50)`を置く前に最もよい行動はどれですか?", opts: ["とにかくfitしてR-hatだけを見る", "betaの単位を確認し、prior predictiveで得点と傾きを描く", "betaを必ず0へ固定する", "標本平均をprior平均にする"], ans: 1, why: "50は睡眠1時間あたり50点の差を意味します。許容する含意を、応答の単位と事前予測で確認します。", hint: "数値の大きさだけでなく、係数が持つ単位を考えます。" },
    { id: "imitate-q02", revision: 1, k: "choice", q: "`normal(0, 5)`から`normal(0, 1)`へbetaのpriorだけを替えると、事前予測の回帰線はどう変わりやすいですか?", opts: ["傾きが0付近へ集中し、極端な傾きが減る", "得点が必ず70点になる", "sigmaが必ず小さくなる"], ans: 0, why: "変更したのは傾きbetaのpriorだけです。interceptやsigmaを直接変える設定ではありません。", hint: "どのparameter classを変更したかに注目します。" },
    { id: "imitate-q03", revision: 1, k: "reflect", q: "prior predictiveで120点の複製得点が少数、−20点が多数出ました。次に調べることを二つ書いてください。", minLength: 55, rubric: ["priorの尺度を調べる提案がある", "正規尤度と得点の支持範囲の関係を検討している", "別familyや別の生成過程を具体的に候補にしている"], example: "interceptとsigmaのpriorが複製得点をどの程度広げているかを分けて調べる。得点が上下限に張り付く観測過程なら、正規尤度以外の生成過程も候補にする。" },
    { id: "imitate-q04", revision: 1, k: "reflect", q: "「default priorを使ったのでpriorの影響はない」と書いた報告へ、短いレビューコメントを書いてください。", minLength: 55, rubric: ["defaultが影響ゼロの証拠でないと述べている", "parameter classまたはpriorの尺度の確認を求めている", "事前予測または代替priorとの比較を求めている"], example: "default priorについても、データ尺度での意味、properかどうか、事前予測を示してください。少なくとも一つの合理的な代替priorで、主要な事後量がどう変わるかも確認してください。" },
    { id: "imitate-q05", revision: 1, k: "choice", q: "二つの合理的なpriorでP(beta > 0 | data)がともに0.97でした。ここから「睡眠を1時間増やす介入は得点を改善する」と書けますか?", opts: ["書ける。prior感度が低いから", "書けない。これは観測された睡眠と得点の条件付き関連であり、因果には別の仮定が要る", "書ける。0.97は95%より大きいから"], ans: 1, why: "priorへの頑健性は重要ですが、交絡や逆因果を解決しません。介入効果には研究設計と因果仮定が必要です。", hint: "関連の推定と、介入の効果の推定を分けます。" },
  ],
  practice: {
    title: "成果物チェック",
