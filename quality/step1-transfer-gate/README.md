# STEP 1 独立転移の公開後フィードバック

## 目的

L16まで終えたR初学者が、中心課題の列名や完成コードをそのまま再生するのではなく、別の注意切替データへ品質検査・整形・集計・保存・限定付き解釈を移せるかについて、公開後に任意でフィードバックを得ます。
...
現時点の状態は`NOT RUN`です。文書やテストが存在するだけで`OBSERVED`にしません。記録が得られた場合は問題の重要度、修正所有者、反証条件を残しますが、未実施・少人数・`NOT RUN`は公開・教材完成・品質判定を止めません。
...
`status.json`の`decision`を手で`OBSERVED`へ変えるだけでは通りません。実施した場合は対象版、TR01〜TR05の匿名証拠、記録の妥当性を機械検査します。これは記録の信頼性を確保するためであり、人数・未解決項目・独立レビューを公開のPASS条件にはしません。プロトコル汚染は`INVALID`、外部要因や証拠不足は`BLOCKED`として、未実施と区別します。
