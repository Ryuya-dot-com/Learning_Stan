...
- Stan編は [非公開教材パック](content/stan/README.md) でL34–L41を設計し、L34「実行経路とデータ契約」・L35「ブロック、型、制約」・L36「生成過程を対数密度へ翻訳する」・L37「CmdStanRで再現可能に実行する」・L38「推定値より先に診断を読む」・L39「generated quantitiesでモデルを検査・比較する」の受講用原稿と理解問題30問まで実装しました。単回帰の縦切り、8単元32課題の文法反復、分布・リンク関数の可視化、切断モデル、PSIS-LOOによる予測モデル比較も検証中ですが、公開アプリにはまだ含まれません
