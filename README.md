...
- Stan編は [非公開教材パック](content/stan/README.md) でL34「実行経路とデータ契約」からL41「自分のモデルを設計・診断・報告する」までの受講用原稿8本と理解問題40問を実装しました。対象commitの静的検証・クリーンCI、単回帰・切断・PSIS-LOOの再検証、L40の弱・強情報比較が完了し、[Stan Release Gate](quality/stan-release-gate/README.md)は`SRG02`〜`SRG05`が`PASS`（4/10）です。Foundation Gate、独立レビュー、初学者観察、遅延保持などが未完了のため、全体は`BLOCKED`で公開アプリにはまだ含めません
