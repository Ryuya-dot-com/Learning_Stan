...
export const REQUIRED_STEP2_EVIDENCE_IDS = Array.from(
  { length: 5 },
  (_, index) => `S2${String(index + 1).padStart(2, "0")}`
);
...
export function deriveStep2ObservationDecision(status) {
  ...
  const allObserved = REQUIRED_STEP2_EVIDENCE_IDS.every(
...
export function validateStep2ObservationStatus(status) {
  const errors = [];
  if (!isRecord(status)) return ["ルートはobjectである必要があります"];
  if (status.schemaVersion !== 1) errors.push("schemaVersionは1である必要があります");
  if (!/^S2O-[A-Z0-9-]+$/.test(status.gateId || "")) {
    errors.push("gateIdはS2O-で始まる必要があります");
  }
  ...
  if (!Array.isArray(status.evidence)) {
    ...
    const missing = REQUIRED_STEP2_EVIDENCE_IDS.filter((id) => !ids.includes(id));
    const unknown = ids.filter((id) => !REQUIRED_STEP2_EVIDENCE_IDS.includes(id));
    ...
    for (const item of status.evidence) {
      ...
      if (item.status === "OBSERVED" && item.artifacts.length === 0) {
  ...
  if (!Array.isArray(status.issues)) {
    ...
    for (const issue of status.issues) {
      ...
      if (issue?.status === "OPEN" && issue?.severity === "P2") {
        ...
        if (!/^\d{4}-\d{2}-\d{2}$/.test(issue.due || "")) {
          errors.push(`${issue.id}: 未解決P2にdueが必要です`);
        }
  ...
  const derived = deriveStep2ObservationDecision(status);
  ...
  if (status.decision === "OBSERVED") {
    ...
    if (!participantRecordsComplete(status)) {
      errors.push("OBSERVEDには適格な匿名観察記録3件以上が必要です");
    }
    if (!hasIndependentReview(status)) {
      errors.push("OBSERVEDには主実装者と異なる独立レビュー署名が必要です");
    }
    if (!hasText(status.decisionRecord)) errors.push("OBSERVEDにはコホート判断記録が必要です");
    if (openIssues(status, ["P0", "P1"]).length > 0) {
      errors.push("OBSERVEDには未解決P0・P1が0件である必要があります");
    }
  }
  return errors;
}

export function summarizeStep2ObservationStatus(status) {
  const evidence = Object.fromEntries([...EVIDENCE_STATES].map((state) => [state, 0]));
  for (const item of status.evidence || []) {
    if (item.status in evidence) evidence[item.status] += 1;
  }
  return {
    decision: deriveStep2ObservationDecision(status),
    evidence,
...
export function loadStep2ObservationStatus(path) {
...
function runCli() {
  ...
  const path = resolve(pathArg || "quality/step2-observation-gate/status.json");
  let status;
  try {
    status = loadStep2ObservationStatus(path);
  } catch (error) {
    console.error(`STEP 2初心者観察の状態を読めません: ${error.message}`);
    process.exitCode = 2;
    return;
  }

  const errors = validateStep2ObservationStatus(status);
  if (errors.length > 0) {
    console.error("STEP 2初心者観察の状態が不正です:");
    for (const error of errors) console.error(`- ${error}`);
    process.exitCode = 2;
    return;
  }

  const summary = summarizeStep2ObservationStatus(status);
  console.log(`STEP 2 Learning Observation: ${summary.decision}`);
  console.log(
    `Evidence: OBSERVED ${summary.evidence.OBSERVED}/${REQUIRED_STEP2_EVIDENCE_IDS.length}, ` +
