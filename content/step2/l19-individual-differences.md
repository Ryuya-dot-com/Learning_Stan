...
完成条件は、線を多く描くことではありません。これまでに作った2つのCSVを入力として次の図を再生成し、線が誰と誰を結んでいるか、条件分布図と何が違うか、どこまで結論できるかを説明できることです。
...
## 2. 条件分布図と対応図は答える問いが違う

前のレッスンで作った箱ひげ＋全点図は、各条件の中央値、四分位、参加者間の分布を読みやすくします。しかし、congの点とincongの点のどれが同じ参加者かは分かりません。

このレッスンでは、同じIDの2条件を1本の線で結びます。線の傾きから各参加者の変化方向を、縦方向の長さから差の大きさを追跡できます。条件内分布と参加者内変化は別の問いなので、2つの図を補完的に使います。
...
生成画像は2100×1500 pxです。保存後は、24本すべてが2点を結ぶこと、48点が入力の縦値と一致すること、congからincongへ読んだ線の差が差CSVと一致することも確認します。
