...
L17〜L19で作った2図・3CSV・探索メモを、空のRセッションから1本の入口スクリプトで再生成します。そのうえで、第三者が次の4点を追跡できる最終分析パックにします。
...
完成版は`examples/step2_report_and_transfer.R`です。L17〜L19の完成版Rスクリプトと同じフォルダへ置き、空のRセッションで実行します。
...
この指定では横が`7 × 300 = 2100 px`、縦が`5 × 300 = 1500 px`です。`plot = condition_plot`を省略すると「最後に表示した図」へ依存するため、別の図を保存しても気づきにくくなります。ファイル名、plot、寸法、単位、dpi、背景を1組の**図の出力契約**として扱います。

## 2. 入口を1本にする

4本のスクリプトを手で順番に選ぶと、L18だけを先に実行したり、古いCSVを読み込んだりできます。L20では実行順をコードにします。
...
`file.exists()`だけでは、空ファイルや古い成果物も通ります。完成版では行数、参加者数、差の方向、画像寸法、メモの節、ファイルサイズ、チェックサムも検査します。manifestはConsoleへ表示し、成果物を1つ増やすためのファイルにはしません。最終契約はあくまで2図・3CSV・探索メモです。
...
L18とL19は同じ主張を重複させる図ではありません。条件内分布と参加者内対応という別の問いに答え、互いの死角を補います。p値、信頼区間、比較対照、標本抽出設計などがない段階で、どちらかを「効果の証明」に使ってはいけません。
