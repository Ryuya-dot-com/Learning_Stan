...
このディレクトリは、公開アプリのSTEP 6を支える設計・実行・検証単位です。L34〜L41と理解問題40問はStan Release Gateの必須6/6を満たしたうえでベータ公開し、各レッスンにはフィードバックを反映した4段階の反復練習と成果物チェックを接続しています。初学者観察と遅延保持は公開後も継続する改善証拠です。
...
現行の32文法課題を実行可能な核とし、公開アプリではL34〜L41の各レッスンへ`まねる → ひとつ変える → 見ずに作る → 別の場面で使う`の4段階練習を追加した。原稿内の`読む・予測する → 穴埋めする → 一部を変える → 意図的なエラーを直す → 見本なしで再現する → 時間を置いて別文脈へ移す`という6接触と合わせ、完成例の記憶だけで通過しない構造にしている。
...
公開判断の正本は`quality/stan-release-gate/status.json`です。対象教材は静的検証とクリーンCI、既存runtime証拠の再検証、L40の弱情報・強情報centered / non-centered比較、所有者による公開範囲確認、最終判断の必須6/6を満たしました。Foundation Gate、初学者観察、L41後7〜14日の保持は公開を止めない改善証拠として追跡し、未実施の効果を主張しません。「練習問題の反復を増やしてほしい」という感想は、公開アプリの各レッスンに4段階練習を追加する形で反映しました。
