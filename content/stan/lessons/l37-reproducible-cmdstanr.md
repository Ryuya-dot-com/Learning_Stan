...
教材リポジトリには、単回帰をR 4.6.1、CmdStanR 0.9.0、CmdStan 2.39.0で実行した`validation.json`があります。これは教材例の実行証拠であり、あなたのPCで実行できた証拠ではありません。自分の初回結果を上書きせずに残します。
...
完成スクリプトを閉じ、環境記録からCSV・入力データ・fit・実行記録の保存までを白紙で再構成します。初回コードを残した後に公式資料を参照し、修正版と支援量を記録します。
...
L37の完了は、サンプリング画面が最後まで進んだことだけでは判定しません。別の人が「何を、どの環境で、どの設定で実行し、どのファイルと診断を得たか」を追跡でき、同じ契約を別の応答型へ移せることを証拠として残します。
