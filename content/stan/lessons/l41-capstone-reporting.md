...
理解問題への正答や自己チェックだけで修得とは判定しません。提出では、計画、初回コード、初回出力、修正版、支援内容を別ファイルとして保存します。
...
L41原稿と理解問題の完成は、学習者が卒業制作を達成した証拠ではありません。卒業制作の提出が揃っても、教材を学習アプリへ公開できるとは限りません。

教材のベータ公開には、対象commitのクリーンCI、runtime証拠、公開範囲確認、所有者判断、未解決P0・P1ゼロが別に必要です。第三者の感想、初学者観察、遅延保持は改善のため継続しますが、未実施だけでは公開を止めません。学習者の修得判定では、初回提出、修正版、支援水準、直接評価を分けます。

さらに`sr41d`はL41修了後7〜14日に行う未見転移です。L41の直後に卒業制作を完成しても、delayed retentionを達成したことにはなりません。この間隔と合格閾値はpilot前に固定しない方針であり、最初の適格な初学者3名の保持率、支援量、完遂時間を確認して再検討します。
...
L41修了後の`sr41d`で、7〜14日を置いて別の応答型へ移します。完成コードやfacilitator参照モデルを見る前に初回提出を固定し、公式リファレンス利用、支援、修正を別々に記録します。
...
11. **L41直後の完成を遅延保持の証拠にする**
    `sr41d`は別日程・別文脈・別記録で行います。
...
自分の主研究と異なる未見文脈について、データ契約から限定付きreportまでを1つの入口で再生成し、第三者へ引き渡します。L41直後の課題と`sr41d`の遅延課題は別成果物・別日程にします。
...
これらは提出に必要な証拠項目です。最終的な修得の合格閾値はpilot前に数値固定せず、対象者観察と得られた第三者フィードバックを参考に定めます。ベータ公開時点で観察が未実施なら、学習効果や保持効果を確認済みとは表現しません。
