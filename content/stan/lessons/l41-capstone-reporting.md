...
このレッスンのダウンロード欄から、参加者×刺激の研究ケース実践パックを取得できます。L30の群構造、L32の観測過程、L33の予測単位を、推定・診断・予測・報告まで一周します。READMEから始め、`starter/protocol.md`を結果を見る前に記入します。

| 問い | 実行する分析 | 答えを支える成果物 |
|---|---|---|
| 条件差はどの尺度でどれだけあるか | `run.R`の3モデル | 条件効果・診断・prior感度分析 |
| 新参加者・新刺激へ何を一般化できるか | 群ごとの保留と4種類の予測 | 分割ID・対象別score・予測区間 |
| brmsとStanで同じモデルを推定したか | `equivalence.R` | 尤度・切片priorの対応・MCSEを伴う予測比較 |
| 観測方法の仮定で答えが変わるか | `observation.R` | 打ち切り・測定誤差の比較と限界 |

中心ケースの条件係数は対数オッズ差です。

$$
\log\frac{p_{pir}}{1-p_{pir}}=\alpha+\beta x_{pir}+a_p+b_i.
$$

βを正答率差と読まず、どの参加者・刺激について平均するかを決めて予測確率へ変換します。モデルの式、予測対象、報告の尺度を対応させてください。

完成スクリプト、意味に問題のある比較例、未完成のスターター、別データの転移課題を同梱しています。実行成功だけで修了にせず、第三者が分析行・estimand・停止判断・主張の範囲を追える報告を提出します。
