...
自己採点だけで修得とは判定しません。初回の診断出力、判断、根拠、変更、再実行結果を上書きせず残します。
...
教材の`validation.json`には、R 4.6.1、CmdStanR 0.9.0、CmdStan 2.39.0で単回帰を4 chain、warmup 1000、sampling 1000として実行した証拠があります。
...
L38の完了は、閾値を暗記したことだけでは判定しません。診断の対象と限界を説明し、必要なら推定値の解釈を止め、次の調査と再実行を1つの証拠系列として残します。
