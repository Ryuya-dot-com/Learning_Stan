...
このモデルの母集団では、Var(w)=τ²+s²、Cov(w,y)=βτ²です。同じ観測データに対してsを大きく仮定すると、説明変数の分散のうち測定誤差へ割り当てる部分が増え、潜在変数の分散τ²を小さくする方向へ働きます。同じ共分散を説明するにはβが大きくなるため、表の変化を理解できます。実際の事後分布は標本の不確実性とpriorにも依存し、このモーメントの関係へ観測値を代入しただけの推定ではありません。
...
σ<0.1は、このデータの単位で境界付近への集中を見るための記述的な指標です。診断の停止基準には使っていません。
